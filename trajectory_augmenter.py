...
class TrajectoryAugmenter():
    ...
    def _aug_nodes(self,
        ...
        for i in range(
            ...
            obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped, loss_mask, V_obs_, A_obs, V_tr_, A_tr,_,_ = next(
                iter(self.data_loader))
            # print('in aug nodes----------------------')
            # print("V_obs_.shape", V_obs_.shape)
            # print("V_tr_.shape", V_tr_.shape)
        ...
        V_obs = torch.cat(V_obs_lst, dim=2)##add the ped_num dimension
        # print("V_obs.shape in aug node----------------", V_obs.shape)
    ...
    def _aug_rot(self, obs_traj, pred_traj_gt):
        ...
        # print('full_traj.shape',obs_traj.shape)##[batch,ped_num,2,obs_len]
        # print('pred_traj_gt.shape',pred_traj_gt.shape)##[batch,ped_num,2,pred_len]
        full_traj = torch.cat((obs_traj, pred_traj_gt), dim=-1)
        # print('full_traj.shape',full_traj.shape)##[batch,ped_num,2,obs_len+pred_len]
        # print('full_traj.transpose(2,3) shape',full_traj.transpose(2,3).shape)
        full_traj = torch.matmul(
            rot_matrix,
            full_traj.transpose(2,
                                3).unsqueeze(-1)).squeeze(-1).transpose(2, 3)
        # print('full_traj.shape after rot:',full_traj.shape)
        return self.abs_to_rel_split(full_traj, full_traj, self.split_time)
